...
function carga_todos() {
    var url = "http://localhost/Service/Service.asmx/GetUsuariosReportan";
    $.getJSON(url, {
        format: "json"
    })
        .done(function (data) {
    ...
    var url2 = "http://localhost/Service/Service.asmx/GetUsuariosReportados";
    $.getJSON(url2, {
        format: "json"
    })
        .done(function (data2) {
...
function correo() {
    if ($("#from").val() != "") {                
        var url2 = "http://localhost/Service/Service.asmx/GetCorreoReportados";
        $.getJSON(url2, {
            correo: $("#from").val(),            
            format: "json"
        })
            .done(function (data2) {
                ...
                $("#reportados").val(result);
            }).fail(function (jqxhr, textStatus, error) {
                var err = textStatus + "," + error;
                console.log("Request Failed:" + err);
            });

        var url3 = "http://localhost/Service/Service.asmx/GetCorreoReportan";
        $.getJSON(url3, {
            correo: $("#from").val(),            
            format: "json"
        })
